...
describe('pb-select-feature', () => {
  ...
  it('should use native select instead of paper-dropdown-menu', () => {
    cy.get('pb-select-feature')
      .shadow()
      .find('paper-dropdown-menu, paper-item, paper-listbox')
      .should('not.exist');
  ...
  it('should populate options from items', () => {
    ...
    cy.get('pb-select-feature')
      ...
      .find('select option')
      .eq(0)
      .should('contain.text', 'Option A');
    cy.get('pb-select-feature')
      .shadow()
      .find('select option')
      .eq(1)
      .should('contain.text', 'Option B');
  });
  ...
  it('should set initial selection', () => {
    cy.mount('<pb-select-feature id="feat" name="test"></pb-select-feature>');
    cy.get('#feat').then($el => {
      $el[0].items = [{ name: 'First' }, { name: 'Second' }];
      $el[0].requestUpdate();
