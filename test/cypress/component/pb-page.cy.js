// Cypress CT: pb-page
import { timeout } from 'tom-select/dist/cjs/utils.js';
...
describe('pb-page', () => {
  ...
  function stubLocalesForApp() {
    cy.intercept('GET', '**/i18n/app/en.json', { fixture: 'i18n/common/en.json' });
    cy.intercept('GET', '**/i18n/app/de.json', { fixture: 'i18n/common/de.json' });
  }
  ...
  describe('i18next integration', () => {
    ...
    it('should handle locale-fallback-ns attribute correctly', () => {
      ...
      stubLocalesForApp();
      cy.mount(
        '<pb-page require-language language="en" api-version="1.0.0" locale-fallback-ns="common app"></pb-page>',
      );
      cy.waitForEvent('pb-page-ready', { timeout: 10000 }).then(() => {
        cy.get('pb-page').then($el => {
          const page = $el[0];
    ...
    it('should handle getBestMatchFromCodes fallback behavior', () => {
      ...
      cy.waitForEvent('pb-page-ready').then(() => {
        cy.get('pb-page').then($el => {
          ...
          const bestMatch = instance.services.languageUtils.getBestMatchFromCodes(
            ['zh-CN', 'zh-TW'],
            ['en', 'de'],
          );
    ...
    it.skip('should load translations from default loadPath', () => {
      ...
      cy.get('pb-page').its('0._i18nInstance.services.backendConnector').should('exist');
    ...
    it('should interpolate loadPath template correctly', () => {
      stubLocales();
      cy.intercept('GET', '**/i18n/common/de.json', { fixture: 'i18n/common/de.json' }).as(
        'loadDe',
      );
      ...
      cy.get('pb-page')
        ...
        .then(instance => {
          ...
          cy.wrap(instance.t('document.contents')).should('exist').and('be.a', 'string');
        });
    });
    ...
    it('should handle crossDomain configuration', () => {
      stubLocales();
      cy.mount(
        '<pb-page require-language language="en" api-version="1.0.0" endpoint="https://example.com"></pb-page>',
      );
      cy.waitForEvent('pb-page-ready');
      cy.get('pb-page')
        ...
        .should('exist')
        .then(backendOptions => {
          cy.wrap(backendOptions.crossDomain || backendOptions.requestOptions?.credentials).should(
            'exist',
          );
        });
    });
    ...
    it('should use chained backend when locales property is set', () => {
      stubLocales();
      cy.intercept('GET', '**/custom/i18n/app/en.json', { fixture: 'i18n/common/en.json' }).as(
        'loadCustom',
      );
      cy.mount(
        '<pb-page require-language language="en" api-version="1.0.0" locales="https://example.com/custom/i18n/{{ns}}/{{lng}}.json"></pb-page>',
      );
      cy.waitForEvent('pb-page-ready');
      cy.get('pb-page')
    ...
    it('should fallback from language-region to language only', () => {
      ...
      cy.mount(
        '<pb-page require-language language="en-US" api-version="1.0.0" fallback-language="en"></pb-page>',
      );
    ...
    it('should handle 404 errors gracefully', () => {
      ...
      cy.mount(
        '<pb-page require-language language="xx" api-version="1.0.0" fallback-language="en"></pb-page>',
      );
      cy.wait('@load404');
    ...
    it('should handle network errors without crashing', () => {
      ...
      stubLocales();
      cy.mount(
        '<pb-page require-language language="en" api-version="1.0.0" fallback-language="en"></pb-page>',
      );
      cy.get('pb-page', { timeout: 5000 }).should('exist').its('0').should('exist');
    });
    ...
    it('should send appropriate request headers', () => {
      ...
      cy.mount('<pb-page require-language language="en" api-version="1.0.0"></pb-page>');
      cy.waitForEvent('pb-page-ready');
      cy.get('pb-page').its('0._i18nInstance').should('exist');
    });
    ...
    it('should load new language when language changes', () => {
      ...
      cy.mount('<pb-page require-language language="en" api-version="1.0.0"></pb-page>');
      cy.waitForEvent('pb-page-ready');
      cy.get('pb-page').its('0._i18nInstance.language').should('equal', 'en');

      cy.document().then(doc => {
        doc.dispatchEvent(
          new CustomEvent('pb-i18n-language', {
            detail: {
              language: 'de',
              key: '__default__',
            },
      ...
      cy.get('pb-page').its('0._i18nInstance.language').should('equal', 'de');
    });
    ...
    it('should configure backend options correctly', () => {
      ...
      cy.get('pb-page')
        ...
        .then(backendOptions => {
          ...
          cy.wrap(backendOptions.crossDomain || backendOptions.requestOptions).should('exist');
        });
    });
    ...
    it('should handle concurrent translation loading', () => {
      ...
      cy.get('pb-page')
        .its('0._i18nInstance')
        .then(instance => {
          cy.wrap(instance.options.ns).should('include', 'common').and('include', 'app');
          cy.wrap(instance.t('document.contents')).should('exist').and('be.a', 'string');
        });
    });
  });
