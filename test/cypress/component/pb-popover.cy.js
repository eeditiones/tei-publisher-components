...
describe('pb-popover', () => {
  it('should show alternate content on click when persistent', () => {
    cy.mount(
      '<pb-popover id="pop" persistent><span slot="default" id="tr">Trigger</span><template slot="alternate"><div id="alt">Alt Content</div></template></pb-popover>',
    );
  ...
  it('should load remote content when remote is set', () => {
    cy.mount(
      '<pb-popover id="pop" persistent remote="/remote.html"><span slot="default" id="tr">Trigger</span><template slot="alternate"><div>Loading...</div></template></pb-popover>',
    );
    // Stub fetch to return HTML
    cy.window().then(win => {
      cy.stub(win, 'fetch').callsFake(() =>
        Promise.resolve(
          new win.Response('<div id="remote">Remote Content</div>', {
            status: 200,
  ...
  it('should show popover on hover when trigger=mouseenter', () => {
    cy.mount(
      '<pb-popover id="pop" trigger="mouseenter"><span slot="default" id="tr">Hover</span><template slot="alternate"><div id="alt2">Hover Content</div></template></pb-popover>',
    );
