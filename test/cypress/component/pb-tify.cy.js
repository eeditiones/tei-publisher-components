  ...
  describe('error handling', () => {
    ...
    it('should emit pb-tify-error event on manifest failure', () => {
      ...
      cy.get('pb-tify').then($el => {
        ...
        // Wait for error to be handled
        cy.get('.pb-tify-error', { timeout: 5000 })
          .should('exist')
          .then(() => {
    ...
    it('should handle URL-based navigation with navigation state', () => {
      ...
      cy.get('pb-tify').then($el => {
        ...
        cy.wait(200).then(() => {
          ...
          expect(element._isNavigationActive('url') || element._navigationState === null).to.be
            .true;
        });
      });
    });
    ...
    it('should allow checkPageChange when no recent commit', () => {
      ...
      cy.get('pb-tify').then($el => {
        ...
        element._tify = {
          app: {
            $root: {
              items: [{ id: 'canvas1', type: 'Canvas', label: { none: ['001'] } }],
            },
          },
          viewer: {
            currentPage: () => 0,
  ...
  describe('wouldDowngrade protection', () => {
    it('should prevent committing older page when registry has newer page', () => {
      ...
      cy.get('pb-tify').then($el => {
        ...
        const registryPageNum = registryState.id
          ? parseInt(registryState.id.match(/_(\d{2,3})\./)?.[1] || '0', 10)
          : null;
        const canvasPageNum = parseInt(canvas.label.none[0], 10);
        const wouldDowngrade =
          registryPageNum !== null && canvasPageNum !== null && canvasPageNum < registryPageNum;
    ...
    it('should prevent downgrading when recent commit has newer page', () => {
      ...
      cy.get('pb-tify').then($el => {
        ...
        const canvasPageNum = parseInt(canvas.label.none[0], 10);
        const wouldDowngrade =
          lastCommitPageNum !== null && canvasPageNum !== null && canvasPageNum < lastCommitPageNum;
            ...
            setTimeout(() => {
              ...
              if (newPages && Array.isArray(newPages)) {
                ...
                if (newPage && newPage !== lastKnownPage && newPage > 0) {
                  ...
                  element._setNavigationState(
                    'user',
                    newPage,
                    `A-N-38_${String(newPage).padStart(3, '0')}.jpg`,
                  );
