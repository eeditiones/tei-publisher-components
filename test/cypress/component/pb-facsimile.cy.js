...
describe('pb-facsimile', () => {
  it('can be opened in fullscreen without removing elements from body', () => {
    cy.mount(`
      <div>
        <pb-facsimile default-zoom-level="0">
    ...
    cy.get('pb-facsimile').then($el => {
      const facsimile = $el[0];

      const requestFullscreen = cy.stub().as('requestFullscreen').resolves();
      const fakeElement = document.createElement('div');
      fakeElement.requestFullscreen = requestFullscreen;

      const fakeButton = { tooltip: 'Toggle full page', onRelease: null, raiseEvent: cy.stub() };
      facsimile.viewer = { element: fakeElement, buttonGroup: { buttons: [fakeButton] } };

      facsimile._overrideFullscreenButton();
      fakeButton.onRelease();
    });
  ...
  it('should navigate to target page and add overlay for annotation', () => {
    ...
    cy.get('pb-facsimile').then($el => {
      ...
      facs._facsimiles = [{ getImage: () => 'img1.jpg' }, { getImage: () => 'img2.jpg' }];
      ...
      cy.get('pb-facsimile').then($facsimile => {
        ...
        cy.wait(20).then(() => {
          ...
          cy.get('@viewerOpen').then(stub => {
            ...
            expect(callArgs.length, 'Negative test: should be exactly 1, not 3').to.equal(
              1,
            ...
            expect(facsimile._facsimiles.length).to.equal(
              3,
              'All 3 links should be in _facsimiles array',
            );
            expect(callArgs.length).to.equal(
              1,
