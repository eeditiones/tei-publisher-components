...
describe('pb-navigation', () => {
  ...
  it('should emit pb-navigate on keyboard shortcut', () => {
    cy.mount(`
      <div>
        <pb-navigation keyboard="right" direction="forward"></pb-navigation>
        <input type="text" />
      </div>
    `);

    const received = new Cypress.Promise(resolve => {
      document.addEventListener('pb-navigate', resolve, { once: true });
    });
    ...
    cy.wrap(received).its('detail.direction').should('equal', 'forward');
  });

  it('should not emit pb-navigate when an input is focused', () => {
    cy.mount(`
      <div>
        <pb-navigation keyboard="right" direction="forward"></pb-navigation>
    ...
    const stub = cy.stub().as('pbNavigate');
    cy.document().then(doc => {
      doc.addEventListener('pb-navigate', stub);
    });

    cy.get('#focusable').focus();
    ...
    cy.get('@pbNavigate').should('not.have.been.called');
  });
});
