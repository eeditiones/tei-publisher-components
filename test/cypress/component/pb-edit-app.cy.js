...
describe('pb-edit-app', () => {
  ...
  it('should render native buttons instead of paper-button', () => {
    ...
    cy.get('pb-edit-app')
      .find('paper-input, paper-dropdown-menu, paper-listbox, paper-checkbox')
      .should('not.exist');
    cy.get('pb-edit-app').find('button.pb-button').should('have.length.at.least', 1);
    cy.get('pb-edit-app').find('#submit').should('have.attr', 'type', 'button');
  });
});
