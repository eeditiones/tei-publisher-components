...
describe('pb-i18n e2e', () => {
  ...
  it('should load and render default language strings', () => {
    cy.get('pb-lang').contains('en').click();
    cy.get('[data-value="zh_TW"]').should('be.visible');
  });
  ...
  it('should work with the translate directive', () => {
    // The pb-demo-snippet uses the translate directive
    cy.get('pb-demo-snippet').shadow().find('button').should('not.contain.text', 'demo');
  });
  ...
  it('should load custom translations from multiple namespaces', () => {
    ...
    cy.get('pb-i18n[key="mycomponent.info"]').should(
      'have.text',
      'An information coming from a custom component',
    );
