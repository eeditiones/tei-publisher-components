...
describe('pb-mei e2e', () => {
  ...
  it('should toggle play and pause controls', () => {
    ...
    cy.get('@viewer')
      .shadow()
      .find('#output')
      .should('not.contain.text', 'Loading', { timeout: 30000 });
    ...
    cy.get('@viewer').shadow().find('#player button').eq(1).as('pauseButton');
      ...
      cy.window().then(win => {
        ...
        cy.get('#invalid-viewer').then($el => {
          ...
          if (element.shadowRoot) {
            ...
            expect(
              hasError || hasLoading || isEmpty || hasContent,
              'Component should show error, loading, or be empty (not crashed)',
            ).to.be.true;
          }
        });
      });
