  ...
  it('should deduplicate network requests when all pb-facs-links point to the same image', () => {
    ...
    cy.fixture('iiif/image-info-complete.json').then(imageInfo => {
      ...
      cy.then(() => {
        ...
        expect(requests.length, 'total requests made (should be 1, not 3)').to.equal(
          1,
          'deduplication failed: expected exactly 1 request when 3 links point to the same image, but got ' +
            requests.length,
        );
        expect(uniqueUrls.length, 'unique URLs requested').to.equal(
          1,
  ...
  it('should deduplicate network requests when pb-facs-links point to a small set of different images', () => {
    ...
    cy.fixture('iiif/image-info-complete.json').then(imageInfo => {
      ...
      cy.then(() => {
        ...
        // This test will fail if deduplication is broken
        expect(requests.length, 'total requests made (should be 2, not 3)').to.equal(
          2,
          'deduplication failed: expected exactly 2 requests when 3 links point to 2 unique images, but got ' +
            requests.length,
        );
        expect(uniqueUrls.length, 'unique URLs requested').to.equal(
          2,
  ...
  it('should fail if deduplication is not working (negative test case)', () => {
    ...
    cy.fixture('iiif/image-info-complete.json').then(imageInfo => {
      ...
      cy.then(() => {
        ...
        expect(requests.length, 'negative test: should be exactly 1, not 3').to.equal(
          1,
        ...
        expect(uniqueUrls.length, 'unique URLs should be 1').to.equal(
          1,
