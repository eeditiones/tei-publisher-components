...
describe('pb-view-annotate e2e', () => {
  beforeEach(() => {
    ...
    cy.document().then(document => {
      ...
      script.setAttribute('src', '../src/pb-view-annotate.js');
    ...
    cy.window().then(window => {
      return new Cypress.Promise(async resolve => {
        await window.customElements.whenDefined('pb-view-annotate');
        resolve();
      });
    });
  });
  ...
  describe('annotation functionality and timing', () => {
    it('should handle annotation initialization with proper timing', () => {
      cy.window().then(win => {
        ...
        cy.wait(400).then(() => {
          ...
          expect(pbViewAnnotate.getAttribute('annotations')).to.equal(
            '[{"id":"ann1","color":"#ff0000"}]',
          );
        });
      });
    });
    ...
    it('should handle selection changes with proper timing', () => {
      cy.window().then(win => {
        const pbViewAnnotate = win.document.createElement('pb-view-annotate');
        pbViewAnnotate.setAttribute('id', 'test-annotate');
        pbViewAnnotate.setAttribute('src', 'document1');
        win.document.body.appendChild(pbViewAnnotate);

        cy.get('#test-annotate').should('exist');
        cy.get('pb-view-annotate').shadow().find('#view').should('exist').should('contain', 'END');
        ...
        cy.wait(150).then(() => {
          ...
          expect(pbViewAnnotate).to.exist;
          expect(win.getSelection().getRangeAt(0).toString()).to.equal('Test');
        });
      });
    });
    ...
    it('scrolls markers into view even when a paragraph does not fit into the screen', () => {
      cy.window().then(win => {
        const pbViewAnnotate = win.document.createElement('pb-view-annotate');
        pbViewAnnotate.setAttribute('id', 'test-annotate');
        pbViewAnnotate.setAttribute('src', 'document1');
        win.document.querySelector('main').appendChild(pbViewAnnotate);

        return cy.wrap(pbViewAnnotate);
      });
      cy.get('pb-view-annotate').shadow().find('#view').should('exist').should('contain', 'END');
      cy.get('pb-view-annotate').then(([pbViewAnnotate]) => {
        const occurences = pbViewAnnotate.search('key', ['END']);
        return cy
          .wrap(occurences)
          .should('exist')
          ...
          .then(([occurence]) => {
            pbViewAnnotate.scrollTo(occurence);
          });
      });

      cy.get('div[part="highlight"]').should('exist').should('be.visible');
    });
  });
});
