...
export class PbTify extends pbMixin(LitElement) {
    ...
    this.subscribeTo('pb-show-annotation', ev => {
      if (ev.detail) {
        ...
        if (url && url !== this.manifest) {
          ...
          const pagesArray = Array.isArray(this._initialPages)
            ? this._initialPages
            : [this._initialPages];
          ...
          if (this._tify.ready) {
            (async () => {
              try {
                ...
                console.error('<pb-tify> Error waiting for Tify ready in navigation:', error);
              }
            })();
          }
          return;
            ...
            const waitForRoot = async () => {
              if (this._tify && this._tify.app) {
                ...
                const root =
                  this._tify.app.$root ||
                  this._tify.app.root ||
                  this._tify.app.manifest ||
                  (this._tify.app.$data && this._tify.app.$data.root);
                if (root) {
  ...
  async _onTifyFullyReady() {
    ...
    if (this._tify && this._tify.viewer) {
      ...
      setTimeout(() => {
        ...
        const thumbnailContainer =
          viewer.$el?.querySelector('.tify-thumbnails') ||
        ...
        if (thumbnailContainer) {
          ...
          thumbnailContainer.addEventListener(
                    ...
                    if (canvas) {
                      ...
                      const waitForTifyThenUpdate = async () => {
                        ...
                        if (tifyCurrentPage === newPage) {
                            ...
                            if (targetIndex >= 0 && targetIndex < canvases.length) {
                              ...
                              if (currentCanvas) {
                                ...
                                if (!canvasId && currentCanvas.label) {
                                  const label =
                                    currentCanvas.label.none ||
                                    currentCanvas.label.en ||
                                    currentCanvas.label;
                                  const labelStr = Array.isArray(label) ? label[0] : label;
                                  if (labelStr) {
                                    const pathParts = window.location.pathname.split('/');
                          ...
                          console.warn(
                            '[pb-tify] thumbnail click: max attempts reached, updating registry anyway',
                            {
                              tifyCurrentPage,
                              newPage,
                              attempts,
            ...
            false,
          ); // Use bubble phase - let Tify handle click first
        }
            ...
            if (root) {
              ...
              if (currentPage >= 1 && currentPage <= canvases.length) {
                ...
                if (canvas) {
                  ...
                  const justCommitted =
                    this._lastCommittedId && currentState.id === this._lastCommittedId;
      ...
      return async pageOrPages => {
        ...
        try {
          ...
          if (error instanceof RangeError) {
            console.warn(
              '<pb-tify> setPage RangeError - invalid page:',
              pageOrPages,
              error.message,
            );
    ...
    const setInitialPage = () => {
        ...
        for (let i = 0; i < canvases.length; i++) {
          ...
          if (canvas.label) {
            ...
            if (labelStr) {
              ...
              if (
                state.id === labelStr ||
                state.id.includes(labelStr) ||
                labelStr.includes(state.id)
              ) {
                initialPage = i + 1;
                break;
              }
      ...
      const hasUrlPage =
        (hash && initialPage === validPage) || (state.id && initialPage === validPage);
  ...
  _setupVueStoreWatcher(app, isRetry = false) {
    ...
    if (
      ...
      app.config.globalProperties.$store.options
    ) {
    ...
    if (!store || !store.options) {
      ...
      console.warn(
        '[pb-tify] Vue store not available for watcher setup, will retry with polling:',
        {
          hasApp: !!app,
            ...
            if (teiCanvas && teiCanvas.rendering && teiCanvas.rendering.length > 0) {
              ...
              if (labelStr) {
                for (const teiCanvas of teiPublisherCanvases) {
                  ...
                  if (
                    teiLabelStr === labelStr &&
                    teiCanvas.rendering &&
                    teiCanvas.rendering.length > 0
                  ) {
                    canvas = teiCanvas;
                    break;
                  }
    ...
    store.setPage = pageOrPages => {
      ...
      const oldPage = oldPages && Array.isArray(oldPages) ? oldPages.find(page => page > 0) : null;
    ...
    store.updateOptions = updatedOptions => {
      ...
      setTimeout(() => {
        const newPagesFromOptions =
          updatedOptions && updatedOptions.pages ? updatedOptions.pages : null;
        ...
        if (newPages) {
          ...
          const lastKnownPage =
            lastWatchedPages && Array.isArray(lastWatchedPages)
            ...
            const waitForTifyUpdate = (attempts = 0) => {
              ...
              if (!this._tify || !this._tify.viewer) {
                ...
                if (attempts < maxAttempts) {
                  ...
                  console.warn(
                    '[pb-tify] Viewer not ready after waiting, calling handlePageChange anyway',
                  );
    ...
    const watchStorePages = () => {
      if (!store || !store.options || !store.options.pages) {
        ...
        if (Date.now() % 1000 < 100) {
          // Log roughly once per second when store is inaccessible
      ...
      try {
        this._vueStoreWatcher = app.$watch(
          ...
          (newPages, oldPages) => {
            ...
            const oldPage =
              oldPages && oldPages.length > 0 ? oldPages.find(page => page > 0) : null;
  ...
  _setupTifyErrorWatcher() {
    ...
    const store =
      this._tify.app.config?.globalProperties?.$store ||
  ...
  async _updateUrlFromPage(canvas, force = false) {
    ...
    if (!force && this._navigationState && this._navigationState.isActive && canvasPageId) {
      ...
      if (!this._matchesNavigationTarget(canvasPageId) && this._navigationState.source !== 'user') {
      ...
      if (root) {
        ...
        if ((!canvas.rendering || canvas.rendering.length === 0) && canvas.label) {
          ...
          if (labelStr) {
            for (const manifestCanvas of canvases) {
              const manifestLabel =
                manifestCanvas.label?.none || manifestCanvas.label?.en || manifestCanvas.label;
              const manifestLabelStr = Array.isArray(manifestLabel)
                ? manifestLabel[0]
                : manifestLabel;
      ...
      if ((!canvas.rendering || canvas.rendering.length === 0) && canvas.label) {
        ...
        if (labelStr) {
          ...
          if (root) {
            ...
            if (docPath) {
              const endpoint = this.getEndpoint();
              const renderingUrl = `${endpoint}/api/parts/${encodeURIComponent(
                docPath,
              )}/html?root=${encodeURIComponent(root)}`;
            ...
            console.warn('[pb-tify] _updateUrlFromPage: root not found in lookup map for page', {
              pageNum,
              labelStr,
            });
          }
    ...
    if (rendering && rendering.length > 0) {
      ...
      if (renderingId) {
        try {
          ...
          console.warn(
            '[pb-tify] _updateUrlFromPage: Error parsing rendering URL:',
            renderingId,
            error,
          );
        }
    ...
    setTimeout(() => {
      ...
      if (this._tify && this._tify.viewer) {
        ...
        if (viewer.currentCanvas) {
          ...
          const currentPage =
            typeof viewer.currentPage === 'function' ? viewer.currentPage() : viewer.currentPage;
      ...
      if (rendering && rendering.length > 0) {
        ...
        if (renderingId) {
          try {
            ...
            console.warn(
              '[pb-tify] _emitPbRefresh: Error parsing rendering URL:',
              renderingId,
              error,
            );
          }
  ...
  _getCanvases(root) {
    ...
    const isCanvas = item => {
      return (
        item &&
        (item.type === 'Canvas' || item['@type'] === 'sc:Canvas' || item['@type'] === 'Canvas')
      );
    };
  ...
  _getRootFromApp() {
    ...
    if (this._tify.app) {
      ...
      const root =
        ...
        app.root ||
        app.manifest ||
        (app.$data && app.$data.root) ||
        (app.$data && app.$data.manifest);
      if (root) {
        return root;
      }
