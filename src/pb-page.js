    ...
    if (!this.apiVersion) {
      ...
      try {
        ...
        try {
          ...
          console.error('<pb-page> Failed to fetch API version:', error);
        }
      }
  ...
  firstUpdated() {
    ...
    (async () => {
      try {
        ...
        console.error('<pb-page> i18next init failed:', error);
        if (this.requireLanguage) {
          this.signalReady('pb-page-ready', {
            endpoint: this.endpoint,
            apiVersion: this.apiVersion,
    ...
    this.subscribeTo('pb-i18n-language', async ev => {
      ...
      try {
        ...
        console.error('<pb-page> Failed to change language:', error);
      }
    });
